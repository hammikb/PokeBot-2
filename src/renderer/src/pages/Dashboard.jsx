...
const STATUS_COLOR = { monitoring: 'text-green-400', idle: 'text-gray-500', running: 'text-yellow-400', error: 'text-red-400' }
              ...
              return (
                <div key={t.id} className="flex items-center gap-3 bg-[#1a1a1a] px-3 py-2 rounded text-xs">
                  ...
                  {status === 'idle' || status === 'error'
                    ? <button onClick={() => startTask(t.id)} className="text-green-500 hover:text-green-300 px-1">▶</button>
                    : <button onClick={() => stopTask(t.id)} className="text-yellow-500 hover:text-yellow-300 px-1">⏸</button>}
                </div>
              )
            })}
          </div>
